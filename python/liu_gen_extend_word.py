...
def actualQE(queryList, K, L, topN, model):
    ...
    autualQueryExtendSorted = sorted(autualQueryExtend, key=lambda t: meanSim(t, finalQueryList), reverse=True)
    if len(autualQueryExtendSorted) < topN:
        autualQueryExtendVector = autualQueryExtendSorted
    else:
        autualQueryExtendVector = autualQueryExtendSorted[0:topN]
    autualQueryExtendWord = []

    QEvec=set()
    for qu in queryList:
        if stem(qu.lower()) not in QEvec:
            QEvec.add(stem(qu.lower()))
    liu_autualQueryExtendVec=[]
    for wVec in autualQueryExtendVector:
        word=getWordByVector(model,wVec)
        if stem(word.lower()) not in QEvec:
            QEvec.add(stem(word.lower()))
            liu_autualQueryExtendVec.append(wVec)
    autualQueryExtendVector=liu_autualQueryExtendVec
...
def actualQEwith2_1(queryList, topN, model):
    ...
    finalQueryList = queryVectorList + integrateQueryList
    candidateWordSorted = sorted(candidateWord, key=lambda t: meanSim(t, finalQueryList), reverse=True)
    if len(candidateWordSorted) < topN:
        autualQueryExtendVector = candidateWordSorted
    else:
        autualQueryExtendVector = candidateWordSorted[0:topN]
    autualQueryExtendWord = []
    for vec in autualQueryExtendVector:
        autualQueryExtendWord.append(getWordByVector(model, vec) + "^" + str(meanSim(vec, finalQueryList)))
    return autualQueryExtendWord
